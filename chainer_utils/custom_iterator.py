...
from iterators import sample_uniform
...
class CustomMultiprocessIterator(iterator.Iterator):
    ...
    def __init__(self, dataset, batch_size, repeat=True, shuffle=True,
                 ...
                 n_classes=None, shuffle_method="random"):
        ...
        self._prefetch_loop = _PrefetchLoop(
            ...
            self._interruption_testing, self.n_classes)
    ...
    def __copy__(self):
        other = CustomMultiprocessIterator(
            self.dataset, self.batch_size, self.repeat, self.shuffle,
    ...
    def reset(self):
        ...
        if self.shuffle:
            ...
            elif self.shuffle_method == "uniform":
                labels = [x[1] for x in self.dataset]
                self._order = sample_uniform(labels, self.batch_size)
            elif self.shuffle_method == "uniform_oversample":
                labels = [x[1] for x in self.dataset]
                self._order = sample_uniform(labels, self.batch_size, True)
            else:
                print(self.shuffle_method)
                raise NotImplementedError()
        else:
            self._order = None
...
class _PrefetchLoop(object):
    ...
    def __init__(self, dataset, batch_size, repeat, shuffle,
                 ...
                 _interruption_testing, n_classes, shuffle_method):
        self.dataset = dataset
        self.batch_size = batch_size
        self.repeat = repeat
        self.shuffle = shuffle
    ...
    def _proceed(self):
        ...
        if new_pos < n:
            ...
            if order is None:
                ...
                indices = order[pos:n]  # あまり
                if self.repeat:
                    if self.shuffle_method == "random":
                        order = numpy.random.permutation(len(self.dataset))
                    elif self.shuffle_method == "uniform":
                        labels = [x[1] for x in self.dataset]
                        order = sample_uniform(labels, self.batch_size)
                    elif self.shuffle_method == "uniform_oversample":
                        labels = [x[1] for x in self.dataset]
                        order = sample_uniform(labels, self.batch_size, True)
                    else:
                        print(self.shuffle_method)
                        raise NotImplementedError()
                    indices = \
